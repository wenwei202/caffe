...
For training large-scale DNNs, the following setups may be a good starting point (which are verified by AlexNet in ImageNet):
  ...
  3. During SSL, the first learning rate is critical to get high sparsity, please train it longer with, say, `0.7*K`iterations. The group sparsity increase slowly at the early iterations and will ramp up rapidly in the later iterations. The second learning rate of SSL is important to maintain accuracy.
